import { Patisserie } from "../Models/Patisserie";

export const getPatisseries = () => Patisserie.find({ number: { $gt: 0 }})
  .then(docs => {
    if (!docs[0]) return false
    else return docs;
  })

export const getRandomPatisserie = () => Patisserie.find({ number: { $gt: 0 } })
  .then(docs => {
    const randIndex = Math.floor(Math.random() * docs.length);
    return docs[randIndex];
  });

export const decrementPatisserie = (p) => Patisserie.updateOne({ order: p.order }, { $set: { number: p.number - 1 } });

export const resetPatisserieCount = () => Patisserie.updateMany({}, { $set: { number: 10 } });